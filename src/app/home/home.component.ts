...
export class HomeComponent implements OnInit {
    ...
    setTimeout(
      () => {
        ...
        localStorage.setItem(
          'theme',
          this.themeService.themeSignal().toString(),
        );
      ...
      this.authService.checkAuthStatus().subscribe({
        next: (isAuthenticated) => {
          if (isAuthenticated.status) {
            // this means that the jwt token still valid (or a valid token refresh happened)
  ...
  onToggleChange(event: MatButtonToggleChange) {
    ...
    else this.toggleService.disableEvent(event.source.value);
    // save to storage
    this.toggleService.saveStateToStorage(
      event.source.value,
