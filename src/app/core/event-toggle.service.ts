...
export class EventToggleService {
  ...
  buttonOrder = [
    { k: 'aq20', n: "Ruins of Ahn'Qiraj" },
    { k: 'ony', n: 'Onyxia' },
    { k: 'zg', n: "Zul'Gurub" },
    { k: 'es', n: 'Emerald Sanctum' },
    { k: 'mc', n: 'Molten Core' },
    { k: 'bwl', n: 'Blackwing Lair' },
    { k: 'aq40', n: "Temple of Ahn'Qiraj" },
    { k: 'naxx', n: 'Naxxramas' },
    { k: 'k40', n: 'Tower of Karazhan' },
    { k: 'k10', n: 'Lower Karazhan Halls' },
    { k: 'dmf', n: 'Darkmoon Fair' },
    { k: 'madness', n: 'Edge of Madness' },
  ];
  ...
  enableEvent(e: keyof EVENT) {
    ...
    this.toggleSignals.update(
      (
        events, // events => means take the list (the one side signal) and do something with it
      ) =>
        events.map((event) => {
          // .map will go through every event, modify them and return new list wit all changed events
          ...
          return {
            // so this's object spreading, direct modification (with type assertion) is commented up, both works the same
  ...
  disableEvent(e: keyof EVENT) {
    ...
    this.toggleSignals.update((events) =>
      events.map((event) => ({
        // opening paranthesis here since we'r not doing explicit return, cause there only one statement
